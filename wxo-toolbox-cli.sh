...
#
# Usage: ./wxo-toolbox-cli.sh
#   ENV_FILE=/path/to/.env ./wxo-toolbox-cli.sh   # override .env location
#
set -e
...
[[ -f "$SCRIPT_DIR/VERSION" ]] && WXO_VERSION=$(cat "$SCRIPT_DIR/VERSION" 2>/dev/null | head -1 | tr -d '[:space:]') || WXO_VERSION="1.0.9"
[[ " ${*} " = *" --version "* ]] || [[ " ${*} " = *" -v "* ]] && { echo "WxO-ToolBox-cli v${WXO_VERSION} by mvk"; exit 0; }
...
_select_environment() {
  BREADCRUMB=("Home")
  _print_header "WxO-ToolBox-cli v${WXO_VERSION} by mvk"
  echo ""
...
_import_options() {
  ...
  # Determine default source environment for connection secrets
  local default_conn_env=""
  if [[ "$SYS_DIR" == *"/Exports/"* ]]; then
    local rest="${SYS_DIR#*Exports/}"
    default_conn_env="${rest%%/*}"
  elif [[ "$SYS_DIR" == *"/WxOExports/"* ]]; then
    local rest="${SYS_DIR#*WxOExports/}"
    default_conn_env="${rest%%/*}"
  fi
  [[ -z "$default_conn_env" ]] && default_conn_env="$WXO_ENV"

  echo ""
  echo "  Which environment's connection secrets (.env_connection) should be mapped?"
  echo "  (Press Enter to use default: $default_conn_env)"
  read -p "  Environment name: " env_conn_choice
  env_conn_choice=$(echo "$env_conn_choice" | tr -d '[:space:]')
  [[ -z "$env_conn_choice" ]] && env_conn_choice="$default_conn_env"
  
  local env_conn_opt=""
  [[ -n "$env_conn_choice" ]] && env_conn_opt="--env-conn-source \"$env_conn_choice\""
  ...
  IMPORT_ARGS="--base-dir \"$SYS_DIR\" --env \"$WXO_ENV\" --no-credential-prompt --if-exists \"$if_mode\" --report-dir \"$import_report_dir\" ${validate_opt} ${env_conn_opt}"
  case "$what" in
